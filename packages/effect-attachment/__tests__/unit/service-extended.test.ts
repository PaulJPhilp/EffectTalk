  ...
  describe("Upload validation", () => {
    ...
    it("should reject MIME type not in allowed list", async () => {
      const program = Effect.gen(function* () {
        ...
        const result = yield* Effect.either(
          service.upload("file.txt", Buffer.from("data"), "text/plain")
        );
  ...
  describe("Download edge cases", () => {
    it("should fail to download nonexistent attachment", async () => {
      const program = Effect.gen(function* () {
        ...
        const result = yield* Effect.either(service.download("nonexistent-id"));
  ...
  describe("List filtering edge cases", () => {
    it("should list attachments with no filters", async () => {
      const program = Effect.gen(function* () {
        ...
        yield* service.upload("file1.txt", Buffer.from("data"), "text/plain", {
          chatId: "chat-1",
        });
        yield* service.upload("file2.jpg", Buffer.from("data"), "image/jpeg", {
          chatId: "chat-2",
        });
    ...
    it("should filter by chatId only", async () => {
      const program = Effect.gen(function* () {
        ...
        yield* service.upload("file2.txt", Buffer.from("data"), "text/plain", {
          chatId: "chat-1",
        });
        yield* service.upload("file3.txt", Buffer.from("data"), "text/plain", {
          chatId: "chat-2",
        });
  ...
  describe("Delete edge cases", () => {
    it("should fail to delete nonexistent attachment", async () => {
      const program = Effect.gen(function* () {
        ...
        const result = yield* Effect.either(service.delete("nonexistent-id"));
