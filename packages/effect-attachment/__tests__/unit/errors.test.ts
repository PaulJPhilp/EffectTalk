  ...
  describe("AttachmentNotFoundError", () => {
    ...
    it("should preserve error through pipeline", async () => {
      const program = Effect.gen(function* () {
        ...
        return yield* service.get("missing").pipe(
          Effect.tapError((err) =>
            Effect.logWarning(
  ...
  describe("InvalidAttachmentError", () => {
    ...
    it("should be catchable by tag during upload", async () => {
      const program = Effect.gen(function* () {
        ...
        return yield* service.upload("", Buffer.from("data"), "text/plain").pipe(
...
describe("Error Recovery Patterns", () => {
  it("should recover from AttachmentNotFoundError with fallback", async () => {
    const program = Effect.gen(function* () {
      ...
      const attachment = yield* service
        .get("missing-id")
        .pipe(
    ...
    const program = Effect.gen(function* () {
      ...
      const result = yield* service.get(uploaded.id).pipe(
        Effect.catchTag("AttachmentNotFoundError", () => {
          attempts++
          return attempts < 2
            ? Effect.fail(new AttachmentNotFoundError({ message: "Retry", id: uploaded.id }))
            : Effect.succeed({
                id: uploaded.id,
                filename: "fallback.txt",
                mimeType: "text/plain",
