...
describe("Backend Abstraction", () => {
  ...
  it("should maintain data consistency across operations", async () => {
    const program = Effect.gen(function* () {
      ...
      // Save multiple blobs
      const blob1 = yield* save(backend, Buffer.from("data1"), "text/plain", {
        id: "blob-1",
      });
      const blob2 = yield* save(backend, Buffer.from("data2"), "image/png", {
        id: "blob-2",
      });
...
describe("Complete Workflows", () => {
  ...
  it("should handle batch operations", async () => {
    const program = Effect.gen(function* () {
      ...
      // Batch delete
      yield* Effect.all(metas.map((meta) => deleteBlob(backend, meta.id)));
...
describe("Error Handling Across Operations", () => {
  it("should handle cascading errors gracefully", async () => {
    const program = Effect.gen(function* () {
      ...
      const delResult = yield* Effect.either(
        deleteBlob(backend, "nonexistent")
      );
