  ...
  describe("getMetadata", () => {
    it("should retrieve metadata without data", async () => {
      const program = Effect.gen(function* () {
        ...
        yield* save(backend, Buffer.from("large data content"), "text/plain", {
          id: "meta-test",
        });
    ...
    it("should preserve metadata consistency", async () => {
      const program = Effect.gen(function* () {
        ...
        expect(retrievedMeta.customMetadata).toEqual(
          originalMeta.customMetadata
        );
      }).pipe(Effect.provide(InMemoryBackend.Default));
...
describe("API Composition", () => {
  it("should support complete CRUD workflow via API", async () => {
    const program = Effect.gen(function* () {
      ...
      yield* save(backend, Buffer.from("updated data"), "text/plain", {
        id: "workflow-test",
        overwrite: true,
      });
