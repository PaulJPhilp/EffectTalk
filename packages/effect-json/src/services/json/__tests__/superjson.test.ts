...
import { superjsonBackend } from "../implementations/superjson.js";
...
describe("superjsonBackend", () => {
  describe("parse", () => {
    ...
    it("should parse SuperJSON with Map", async () => {
      ...
      expect(
        (result as { metadata: Map<string, string> }).metadata
      ).toBeInstanceOf(Map);
    });
  ...
  describe("round-trip", () => {
    it("should preserve Date through round-trip", async () => {
      ...
      const stringified = await Effect.runPromise(
        superjsonBackend.stringify(original)
      );
      const parsed = await Effect.runPromise(
        superjsonBackend.parse(stringified)
      );
    ...
    it("should preserve Set through round-trip", async () => {
      ...
      expect(Array.from((parsed as { tags: Set<string> }).tags)).toEqual(
        Array.from(original.tags)
      );
    });
    ...
    it("should preserve Map through round-trip", async () => {
      ...
      expect(
        (parsed as { metadata: Map<string, string> }).metadata
      ).toBeInstanceOf(Map);
      expect(
        Array.from(
          (parsed as { metadata: Map<string, string> }).metadata.entries()
        )
      ).toEqual(Array.from(original.metadata.entries()));
    });
  });
});
