          ...
          if (__metadata.should_validate) {
            ...
            if (result._tag === "Right" && __metadata.round_trip !== false) {
              ...
              const stringified = await Effect.runPromise(
                Json.stringify(UserSchema, result.right)
              );
      ...
      it(`should handle: ${name} - ${__metadata.description}`, async () => {
        ...
        if (data) {
          ...
          if (__metadata.should_validate) {
            ...
            if (
              stringified._tag === "Right" &&
              __metadata.round_trip !== false
            ) {
              // Round-trip test
              const reparsed = await Effect.runPromise(
                Effect.either(
                  Json.parseSuperjson(GenericSchema, stringified.right)
                )
              );
        ...
        if (__metadata.should_parse) {
          if (__metadata.should_validate) {
            ...
            if (result._tag === "Right" && __metadata.round_trip !== false) {
              ...
              const reparsed = await Effect.runPromise(
                Json.parse(ConfigSchema, stringified)
              );
              expect(reparsed).toEqual(result.right);
            }
          }
...
`;
    ...
    it("should guarantee round-trip for SuperJSON with Date", async () => {
      ...
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          const stringified = yield* Json.stringifySuperjson(
            ComplexSchema,
            original
          );
          const reparsed = yield* Json.parseSuperjson(
            ComplexSchema,
            stringified
          );
          return { original, reparsed };
      ...
      expect(result.reparsed.createdAt.toISOString()).toBe(
