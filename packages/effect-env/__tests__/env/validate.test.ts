 ...
 * - validate(): Startup validation for environment schemas
 ...
 * - Helpful debugging output in development
 *
 * Note: validate() is a utility function separate from schema validation in createEnv/createSimpleEnv.
 * Use for optional startup checks; schema validation in creation APIs is automatic.
 */
...
describe("validate", () => {
  ...
  describe("missing required key", () => {
    ...
    it("should return ValidationError in dev mode", async () => {
      const result = await Effect.runPromiseExit(
        validate(exampleSchema, missingSource, { failInProd: false })
      );
    ...
    it("should die in production mode", async () => {
      const result = await Effect.runPromiseExit(
        validate(exampleSchema, missingSource, { failInProd: true })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure") {
  ...
  describe("error extraction - invalid types", () => {
    it("should extract type validation errors", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(typedSchema, invalidSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
    ...
    it("should handle NumberFromString validation errors", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(numberSchema, invalidSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
    ...
    it("should handle BooleanFromString validation errors", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(boolSchema, invalidSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
    ...
    it("should handle Literal validation errors", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(literalSchema, invalidSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
  ...
  describe("error extraction - mixed errors", () => {
    it("should extract both missing and invalid fields", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(mixedSchema, mixedSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
    ...
    it("should generate comprehensive report with multiple errors", async () => {
      ...
      const result = await Effect.runPromiseExit(
        validate(complexSchema, errorSource, { failInProd: false })
      );
      expect(result._tag).toBe("Failure");
      if (result._tag === "Failure" && result.cause._tag === "Fail") {
        const error = result.cause.error as ValidationError;
  ...
  describe("formatValidationReport", () => {
    ...
    it("should truncate long error messages", () => {
      ...
      expect(
        report
          .split("\n")
          .some((line) => line.includes("CONFIG") && line.length < 200)
      ).toBe(true);
    });
