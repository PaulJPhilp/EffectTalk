/**
 * @fileoverview
 * Tests for EnvService functionality.
 *
 * EnvService Architecture (Phase 1 refactor, v0.4.1):
 * - Service definition: EnvService extends Effect.Service<Env<any>>()
 * - Usage: Effect.gen() pattern via EnvService tag
 * - Implementation: makeEnv() creates Env instances
 *
 * Test coverage includes:
 * - get/require methods: accessing environment variables with type safety
 * - Optional fields: handling undefined values and defaults
 * - Type transformations: NumberFromString, BooleanFromString, etc.
 * - Legacy methods: getNumber, getBoolean, getJson (@deprecated)
 * - Error handling: MissingVarError for required undefined values
 *
 * Note: Tests use deprecated fromRecord layer API for simplicity.
 * New code should use createSimpleEnv() or createEnv() instead.
 */

import { Effect, Schema as S } from "effect";
import { describe, expect, it } from "vitest";
import { MissingVarError } from "../../src/env/errors";
...
describe("service", () => {
  ...
  describe("get vs require", () => {
    it("get() succeeds with undefined, require() fails for missing optional field", async () => {
      ...
      const getResult = await Effect.runPromise(
        getProgram.pipe(Effect.provide(layer))
      );
    ...
    it("both succeed for provided required field", async () => {
      ...
      const requireResult = await Effect.runPromise(
        requireProgram.pipe(Effect.provide(layer))
      );
  ...
  describe("getNumber", () => {
    ...
    it("should fail on invalid number", async () => {
      ...
      await expect(
        Effect.runPromise(program.pipe(Effect.provide(layer)))
      ).rejects.toThrow("Invalid number for EXTRA_NUM: abc");
    });
    ...
    it("should fail on missing key", async () => {
      ...
      await expect(
        Effect.runPromise(program.pipe(Effect.provide(layer)))
      ).rejects.toThrow("Environment variable MISSING not found");
    });
  });
  ...
  describe("getJson", () => {
    ...
    it("should fail on invalid JSON", async () => {
      ...
      // JSON parser provides its own error message; just verify it contains the snippet
      await expect(
        Effect.runPromise(program.pipe(Effect.provide(layer)))
      ).rejects.toThrow(/Expected double-quoted property name in JSON.*{/);
    });

    it("should truncate long invalid JSON", async () => {
      const longInvalid =
        '{"name": "test", "data": "' + "x".repeat(100) + '", invalid}';
      ...
      // JSON parser error with truncated snippet
      await expect(
        Effect.runPromise(program.pipe(Effect.provide(layer)))
      ).rejects.toThrow(/Expected double-quoted property name.*\.\.\./);
    });
  });
