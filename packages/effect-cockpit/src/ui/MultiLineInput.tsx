...
export const MultiLineInput: React.FC<MultiLineInputProps> = ({
  ...
  const slashTerm = isSlash ? value.slice(1).split(/\s+/)[0]?.toLowerCase() || "" : "";
  ...
  return (
    <Box flexDirection="column">
      {isSlash && matchingSuggestions.length > 0 && (
        <Box flexDirection="column" borderStyle="round" borderColor="magenta" paddingX={1} marginBottom={0}>
          ...
          <Text dimColor italic>Tab to complete</Text>
        </Box>
      )}
