...
export const VirtualBlockList: React.FC<VirtualBlockListProps> = ({ session, height = 20 }) => {
  // Simple virtualization: Render a window around the focused block
  // If no focus, render the last N blocks (tailing)
  
  const focusedIndex = session.blocks.findIndex(b => b.id === session.focusedBlockId);
            ...
            return (
              ...
              >
                <Box>
                  ...
                  {isFocused && <Text color="cyan" bold> (FOCUSED)</Text>}
                </Box>
