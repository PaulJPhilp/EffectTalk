  ...
  ProcessRuntime.of({
    spawn: (command, args, options) =>
      Effect.gen(function* () {
        ...
        const onExit = Effect.async<{ exitCode: number; signal?: number }, Error>(
          (resume) => {
            child.on("close", (code, signal) => {
              resume(Effect.succeed({ exitCode: code ?? 0, signal: signal === null ? undefined : 0 }));
            });
            child.on("error", (err) => {
        ...
        const resize = (cols: number, rows: number) =>
          Effect.void; // child_process doesn't support resize
