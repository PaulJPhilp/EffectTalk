...
export const SlashCommandsLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const register = (command: SlashCommand) =>
      Ref.update(commands, (current) => ({ ...current, [command.name]: command }));

    const getAvailable = Effect.map(Ref.get(commands), (cmds) => Object.values(cmds));
    ...
    yield* register({
      ...
      execute: () => sessionStore.update((s) => ({ ...s, blocks: [], activeBlockId: null, focusedBlockId: null }))
    });
