...
export interface CommandExecutor {
  readonly execute: (command: string) => Effect.Effect<void, Error, Scope.Scope | SessionStore | BlockService | ProcessRuntime | SlashCommands>;
}
    ...
    return CommandExecutor.of({
      execute: (command: string) =>
        Effect.gen(function* () {
          ...
          yield* sessionStore.update((s) => ({ ...s, activeBlockId: block.id }));
