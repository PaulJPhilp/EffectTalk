...
export const SessionStoreLive = (initialSession: Session) =>
  Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      const updateBlock = (blockId: string, updateFn: (block: Block) => Block) =>
        update((session) => ({
          ...session,
      ...
      const focusNext = update((session) => {
        ...
        return nextBlock ? { ...session, focusedBlockId: nextBlock.id } : session;
      });
      ...
      const focusPrev = update((session) => {
        ...
        return prevBlock ? { ...session, focusedBlockId: prevBlock.id } : session;
      });
