...
export interface Persistence {
  ...
  readonly createSnapshot: (name: string, session: Session) => Effect.Effect<void>;
  readonly listSnapshots: Effect.Effect<ReadonlyArray<{ name: string; createdAt: number }>>;
...
export const PersistenceLive = (dbPath: string) =>
  Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      let dialect: any;
      if (isBun) {
        const { BunSqliteDialect } = yield* Effect.promise(() => import("kysely-bun-sqlite"));
        ...
        const { default: Database } = yield* Effect.promise(() => import("better-sqlite3"));
      ...
      return Persistence.of({
        ...
        listSnapshots: Effect.promise(() => 
            db.selectFrom("snapshots")
              .selectAll()
              .orderBy("createdAt", "desc")
              .execute()
        ),
        loadSnapshot: (name) => Effect.gen(function*() {
